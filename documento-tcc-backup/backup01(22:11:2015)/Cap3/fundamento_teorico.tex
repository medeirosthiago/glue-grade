...
\textit{Data Warehouse} é um armazém centralizado de dados. \textit{Data Warehousing} refere-se à organização dos dados para os tornar disponíveis para análise dos dados. Uma das ferramentas que vem apresentando vantagens em relação ao SQL é a OLAP (\textit{On line Analytical Processing}). Existem similaridades e diferenças entre OLAP e \textit{data mining} mas foge do escopo deste trabalho.
...
Diversos métodos são usados em \textit{data mining} para encontrar respostas ou extrair conhecimento interessante. Pode-se obter os mesmos por meio dos seguintes métodos:
...
\begin{itemize}
	...
	\item Desenvolvimento \textit{backend} para aplicações \textit{web}: É possível criar APIs e interagir com banco de dados. \textit{Frameworks} mais utilizados inclui \textit{Django}, \textit{Flask} e \textit{Pyramid};
