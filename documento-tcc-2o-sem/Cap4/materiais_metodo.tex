...
\begin{itemize}
    ...
    \item Alinhamento de dados e manipulação de dados relacionais para operações de junção e mescla (\textit{join} e \textit{merge}) de conjuntos de dados heterogeneos;
...
Esta biblioteca possui duas estruturas de dados principais: \textit{Series} e \textit{DataFrame}. Estas estruturas não são uma solução universal para todos os problemas, mas provê uma base sólida e de fácil manipulação para a maioria das aplicações com mineração de dados.
...
Protocolos de autenticação são capazes de, simplesmente, autenticar a parte que está se conectando, ou ainda de autenticar a parte que está sendo conectado, assim como se autenticar para ele.
